...
public class NotificationUtils {
    ...
    // COMPLETE (1) Create a constant int value to identify the notification
    ...
    public static void notifyUserOfNewWeather(Context context) {
        ...
        if (todayWeatherCursor.moveToFirst()) {
            ...
            // COMPLETE (2) Use NotificationCompat.Builder to begin building the notification
            ...
            // COMPLETE (3) Create an Intent with the proper URI to start the DetailActivity
            Intent detailIntent = new Intent(context, DetailActivity.class);
            detailIntent.setData(todaysWeatherUri);

            // COMPLETE (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder taskStackBuilder = TaskStackBuilder.create(context);
            taskStackBuilder.addNextIntentWithParentStack(detailIntent);
            PendingIntent resultPendingIntent = taskStackBuilder
                    .getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);

            // COMPLETE (5) Set the content Intent of the NotificationBuilder
            notificationBuilder.setContentIntent(resultPendingIntent);

            // COMPLETE (6) Get a reference to the NotificationManager
            NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

            // COMPLETE (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(NOTIFICATION_ID, notificationBuilder.build());

            // COMPLETE (8) Save the time at which the notification occurred using SunshinePreferences
